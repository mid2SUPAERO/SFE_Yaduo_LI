...
uSamples = np.ones((n_FEM_nodes*6,nSamples)) # POD displacement
gSamples = np.ones((n_gamma_nodes,nSamples)) # POD circulation
...
for iIteration in range(nIterations):
    ...
    for iCandidate in indexCand:
        ...
        q_1 = np.ones((nSamples,))*1000.0
        q_2 = np.ones((nSamples,))*1000.0
        q_e = np.ones((nSamples,))*1000.0
        g_0 = np.ones((nSamples,))*1000.0
        g_1 = np.ones((nSamples,))*1000.0
        g_2 = np.ones((nSamples,))*1000.0
        g_e = np.ones((nSamples,))*1000.0
        ...
        while n < itermax and err > tol:
            ...
            if n < 3:
                ...
                d_g_q = q - q_1
                dd_x_q = d_g_q - d_x_q
                q_0 = q - np.dot(np.dot(d_g_q,dd_x_q)/np.square(np.linalg.norm(dd_x_q)),d_g_q)
                d_x_g = g_1 - g_2
                d_g_g = gr - g_1
                dd_x_g = d_g_g - d_x_g
                g_0 = gr - np.dot(np.dot(d_g_g,dd_x_g)/np.square(np.linalg.norm(dd_x_g)),d_g_g)
            n += 1
        # 5b) Compute error indicator
        toc1 = timeit.default_timer()
        print("Reduced convergence time = "+str(toc1-tic1)+"s")
        print("n = "+str(n))
        plt.plot(np.linspace(2,n,n-1),err_vec[1:])
#        err_mean = np.mean(err_vec[1:])
#        plt.ylim(top=(err_mean  + 1e-4))
#        plt.ylim(bottom=(err_mean - 1e-4))
        plt.grid(True)
        plt.show()
