"""
VERIFICATION ONLINE PHASE aerostruct
This program aims to solve several cases of the POD based reduced model and to
verify that it has the desired behaviour.
...
uV = np.load("../results/Offline/uV.npy")
gV = np.load("../results/Offline/gV.npy")
...
nParam = 6
nLHS = 5
candLHS = np.dot(pMin,np.ones((1,nLHS))) + lhs(nParam,nLHS).T*np.dot((pMax-pMin),np.ones((1,nLHS)))

for iLHS in range(1):
    ##ONLINE TREATMENT
    h_skins = candLHS[0,iLHS]
    h_ribs = candLHS[1,iLHS]
    h_spars_le = candLHS[2,iLHS]
    h_spars_te = candLHS[3,iLHS]
    b = candLHS[4,iLHS]
    S = candLHS[5,iLHS]
    ...
    print("Average Von Mises Stress (mean) = "+str(np.mean(Von_Mises_Stress))+" Pa")
    # 3d) g_est
    ...
    subprocess.call(['D://gmsh-3.0.6/gmsh','../results/Param_wing/g_est.msh','../results/Param_wing/g_est1.msh','../results/Param_wing/g_est2.msh','../results/Param_wing/g_real.msh','-open'])
    subprocess.call(['D://gmsh-3.0.6/gmsh','../results/Param_wing/u_est.msh','../results/Param_wing/u_est1.msh','../results/Param_wing/u_est2.msh','../results/Param_wing/u_real.msh','-open'])
    #subprocess.call(['D://gmsh-3.0.6/gmsh','../results/Param_wing/result_VM_iter.msh','../results/Param_wing/result_VM.msh','-open'])
    print('*********************************************************')