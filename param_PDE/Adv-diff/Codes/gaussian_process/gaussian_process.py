...
class GaussianProcess(BaseEstimator, RegressorMixin):  # pragma: no cover
    ...
    def random_trajectories(self, X, n):
        """
        ...
        Traj : array_like
            Values of the random trajectories discretized at points X
                ...
                """
        # Conditioned covariance matrix at X
#        if self.K==None:
#            K = self.conditioned_covariance_function(X)
#        elif self.K.shape[0]!=X.shape[0]:
#            K = self.conditioned_covariance_function(X)
#        else:
        K = self.K
        # Cholesky factorization of K
        C_n = np.sqrt(self.sigma2) * linalg.cholesky(K, lower=True)
        # Mean prediction
        mean = self.predict(X)
        ...
        Traj = []
        for i in range(n):
            xi = np.random.normal(0.0, 1.0, len(X))
            Traj.append(mean[:, 0] + np.dot(C_n, xi))
        Traj = np.array(Traj)
        return Traj

    def KL_expansion(self, eps, Omega):
        """
        This function computes the KL expansion of the gp by the Nystrom method.
        ...
        Omega : array_like
            Integration domain boundaries (n_features,2)
        Returns
        -------
        KL : array_like
        ...
        """
        # initialisation of the eigenvectors discetized over a sampling test
        X_test = np.linspace(Omega[0, 0], Omega[0, 1], 200)
        # number of integration point
        n_start = 10
        for it in range(10):
            # Simpson rule
            nodes = np.linspace(Omega[0, 0], Omega[0, 1], n_start)
            h = nodes[1] - nodes[0]
            ...
            K = self.conditioned_covariance_function(
                nodes.reshape((len(nodes), 1)))
            sqW = np.sqrt(W)
            B1 = np.dot(K, sqW)
            B = np.dot(sqW, B1)
            ...
            n = np.argwhere(crit < eps)[0, 0]
            # Computation of the eigenvectors approximation and convergence
            # criterion
            Phi_1 = self.eval_KL_vect(
                weights, nodes, eig, vect_star, X_test, n)
            n_start = n_start + 2

        return eig, vect_star, weights, nodes

    def eval_KL_vect(self, weights, nodes, eig, vect, X, n):
        """
        This function computes the eigenvectors from the result
        of the Nystrom method.
        ...
        vect : array_like
            eigenvectors
        X : array_like
            Points where eigenvectors should be discretized
        Returns
        -------
        vect : array_like
            Eigenvectors
        ...
        """
       # Covariance matrix
        nodes_tot = np.concatenate((X, nodes))
        K_tot = self.conditioned_covariance_function(
            nodes_tot.reshape((len(nodes_tot), 1)))
        K_red = K_tot[len(X):, 0:len(X)]
        W = np.zeros((len(nodes), len(nodes)))
        np.fill_diagonal(W, weights)
        sqW = np.sqrt(W)
        B = np.dot(sqW, vect)
        Phi = np.dot(B.T, K_red)
        eig_mat = np.zeros((eig.shape[0], eig.shape[0]))
        np.fill_diagonal(eig_mat, 1.0 / eig)
        Phi = np.dot(eig_mat, Phi)
        Phi = Phi.T
        Phi = Phi[:, 0:n]
        return Phi
